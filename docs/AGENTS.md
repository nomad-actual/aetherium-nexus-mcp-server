...
A [Model Context Protocol (MCP)](https://modelcontextprotocol.io) server built on Node.js that exposes a set of tools to AI clients via Streamable HTTP transport.
...
├── types.ts              # Shared TypeScript types (config, weather, scraper, tools)
├── server/
...
`.github/workflows/release.yml` creates semantic releases:
...
- **bump detection** (first match wins):
  1. **PR title prefix** (conventional-commit style): `major:` → major, `fix:`/`hotfix:` → patch, any other colon prefix (`feat:`, `chore:`, …) → minor
  2. **Branch prefix** (slash-separated): `major/` → major, `fix/`/`hotfix/` → patch, anything else → minor
  3. **Fallback**: minor

  | Example PR title / branch | Bump |
  |---------------------------|------|
  | `major: drop node 24` / `major/drop-node-24` | major |
  | `fix: scraper timeout` / `fix/scraper-timeout` | patch |
  | `feat: weather alerts` / `feat/weather-alerts` / `chore: deps` / no prefix | minor |

- Release notes are auto-generated by `gh release create --generate-notes`; workflows are serialized via a `release` concurrency group.
...
- Prefix the **PR title** with the conventional-commit style prefix (`fix: ...`, `feat: ...`, `major: ...`) — this takes precedence over the branch name.
- Prefix the **branch** with the same prefix + slash (`fix/...`, `hotfix/...`, `major/...`, `feat/...`) as a fallback when the title has no prefix.
