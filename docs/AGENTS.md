...
- **trigger**: PR merged into `main` (the workflow reads the PR's source branch name to decide the version bump)
- **version source**: git tags (`vX.Y.Z`) — do not edit `package.json`'s `version` for releases; it is not kept in sync
- **branch prefix → bump** (prefix must be followed by a colon):

  | Branch prefix | Bump | Example |
  |---------------|------|---------|
  | `major:` | major | `major:drop-node-24-support` → `1.0.0` |
  | `fix:`, `hotfix:` | patch | `fix:scraper-timeout` → `0.1.1` |
  | anything else (`feat:`, `feature:`, `chore:`, no prefix…) | minor | `feat:weather-alerts` → `0.2.0` |
...
Work branches that will be merged to `main` should be named `<prefix>:<slug>` so the release workflow bumps correctly: `major:`, `feat:`/`feature:`, `fix:`, `hotfix:`, or any other `prefix:` (e.g. `chore:`). Remember the colon is required — `feat-foo` does NOT match `feat:` and falls through to the minor default.
